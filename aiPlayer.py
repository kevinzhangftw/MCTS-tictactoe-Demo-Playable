...
class MCTSPolicy(Policy):
	...
	def move(self, starting_state):
		...
		if self.last_move is not None:
			print ('self.lastmove is not None')
			...
			for child in self.digraph.successors(self.last_move):
				...
				if self.digraph.node[child]['state'] == starting_state:
					print('child state is same as starting state')
					...
					if self.digraph.has_edge(self.last_move, child):
						print('link between child and last state')
						exists = True
						starting_node = child
			if not exists:
				print('starting state not in graph, adding now')
		...
		if root not in self.digraph.nodes():
			self.digraph.add_node(self.node_counter,
								  attr_dict={'w': 0,
											 ...
											 'state': root})
			self.node_counter += 1
			print ('root returned because root is not in digraph')
			return root
